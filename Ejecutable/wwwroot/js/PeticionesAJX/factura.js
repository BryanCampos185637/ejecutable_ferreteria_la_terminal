...
function edit(id) {
  $.get("/factura/getFacturaById?id=" + id, function (data) {
    document.getElementById("titulo").innerHTML =
      '<h3 class="modal-title">Factura No. ' + data.nofactura + "</h3>";
    ...
    document.getElementById("infoFactura").innerHTML = `<tbody>
                            ...
                            <tr>
                                ...
                                <td>Nit: ${data.nit}</td>
                                <td>Comisión: $${data.totalcomision.toFixed(
                                  2
                                )}</td>
                            </tr>
                            <tr>
  ...
  $.get("/factura/getDetallePedidoByIidfactura?id=" + id, function (lst) {
    var html = "";
    html +=
      '<table class="table table-hover table-bordered table-responsive-sm table-responsive-md" id = "listProductsFactura">';
...
function addIdFacturaCookie(esOriginal = true) {
  ...
  if (efectivo != "") {
    efectivo = efectivo * 1;
    let total = document.getElementById("total").value * 1;
    if (efectivo >= total) {
      var id = document.getElementById("iidfactura").value;
      $.get(
        "/factura/addIdFacturaCookie?id=" +
          id +
          "&esOriginal=" +
          esOriginal +
          "&efectivo=" +
...
function paintTableFactura(
  link = "/factura/listFactura",
  headboard = [
    "no. factura",
    "tipo comprador",
    "cliente",
  ...
  primaryKey = "iidfactura",
  idModal = "modalFactura",
  optionDelete = false,
  optionEdit = true
) {
  $.get(link, function (data) {
    ...
    for (var c = 0; c < data.length; c++) {
      ...
      if (optionDelete != false || optionEdit != false) {
        ...
        if (optionDelete)
          html +=
            '<a class="btn-sm btn-danger" href="#" onclick="deleteInfo(' +
            objectCurret[primaryKey] +
            ')"><i class="fas fa-trash"></i></a> ';
        if (optionEdit)
          html +=
            '<a class="btn-sm btn-success" href="#" data-toggle="modal" onclick="edit(' +
            objectCurret[primaryKey] +
            ')" data-target="#' +
            idModal +
...
function paintTableCotizacion(
  link = "/factura/listCotizacion",
  headboard = ["no. Cotización", "comprador", "creada por", "total"],
  properties = ["nocotizacion", "nombrecliente", "nombreusuario", "total"],
  primaryKey = "iidcotizacion"
) {
  $.get(link, function (data) {
    var html = "";
    html +=
    ...
    for (var c = 0; c < data.length; c++) {
      ...
      html += "<td>";
      html +=
        '<a class="btn-sm btn-primary" href="#" onclick="imprimirCotizacion(' +
        objectCurret[primaryKey] +
        ')"><i class="fas fa-edit"></i> Imprimir</a>';
      html += "</td>";
      html += "</tr>";
    }
    html += "</tbody>";
