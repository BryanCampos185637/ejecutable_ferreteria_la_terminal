...
function CallTable(tipo) {
    if (tipo == 'bodega') {
        pintarTabla('Lista de bodegas','Puedes editar o eliminar las bodegas ya creadas', '/bodegainventario/listBodega', ['codigo', 'nombre'],
            ['iidbodega', 'nombrebodega'], 'iidbodega', 'bodega');
    } else {
        pintarTabla('Lista del inventariado','Puedes mover los productos entre bodegas o enviarlos a venta', '/bodegainventario/listInventario', ['codigo', 'producto', 'bodega','stock', 'cantidad'],
            ['codigoproducto', 'nombreproducto', 'nombrebodega' ,'nombrestock', 'cantidad'], 'iidinventario', 'inventario');
    }
}
/**
 ...
 */
function pintarTabla(titulo,subtitulo, url, cabecera, propiedades, llavePrimaria, tipo) {
    document.getElementById('tablaGenerica').innerHTML = '<span>Cargando lista...</span>';
    $.get(url, function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            html += '<td>'
            if (tipo == 'bodega') {//si es bodega
                html += '<a class="btn-sm btn-success" href="#" onclick="editarBodega(' + dataActual[llavePrimaria] + ')" data-toggle="modal" data-target="#modalGenerico"><i class="fas fa-edit"></i></a> ';
                html += '<a class="btn-sm btn-danger" href="#" onclick="eliminar(' + dataActual[llavePrimaria] + ',true)"><i class="fas fa-trash"></a>';
            } else {//si es inventario
                if (dataActual['cantidad'] > 0) {//si la cantidad es mayor a 0 se podra mover entre inventarios
                    html += '<a class="btn-sm btn-success" href="#" onclick="moverProducto(' + dataActual[llavePrimaria] + ')" data-toggle="modal" data-target="#modalMoverProducto">Mover <i class="fas fa-people-carry"></i></a> ';
                } else {//de lo contrario se bloquea el boton
                    html += '<button class="btn-sm btn-success">Mover <i class="fas fa-people-carry"></i></button> ';
                }
                html += '<a class="btn-sm btn-danger" href="#" onclick="eliminarInventario(' + dataActual[llavePrimaria] + ')">Eliminar <i class="fas fa-trash"></i></a> ';
                html += '<a class="btn-sm btn-success" href="#" onclick="editarCantidadProducto(' + dataActual[llavePrimaria] + ')" data-toggle="modal" data-target="#modalEditarCantidad" title="Edita las existencias">Editar <i class="fas fa-edit"></i></a> ';
            }
            html += '</td>'
            html += '</tr>'
        }
 ...
 */
function ConfirmarEliminacion(url, EsBodega, texto = '¿Estas seguro que deseas eliminar este registro?', titulo ='Eliminar registro!') {
    Swal.fire({
        title: titulo,
        text: texto,
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.get(url, function (respuesta) {
                if (respuesta == 'ok') {
                    ...
                    if (EsBodega) { CallTable('bodega'); }
                    else { CallTable('inventario'); }
                } else if ("utilizada") {
                    if (EsBodega) {
                        messeges('warning', 'No se pudo eliminar, esta bodega esta en uso', 2500)
                    } else {
                        messeges('warning', 'No se pudo eliminar, este inventario tiene existencias', 2500)
                    }
                } else {
...
function llenarCombo(idBodega) {
    $.get('/bodegainventario/listBodegaSelect?id=' + idBodega, function (data) {
        ...
        $.each(data, function (key, item) {
            html += '<option value="' + item.iidbodega + '">' + item.nombrebodega + '</option>';
        });
...
function moverCantidadProducto() {
    if (!($('#cantidadMover').val() == '' || $('#cbxBodega').val() == '')) {
        ...
        if (cantidad > 0 && cantidad <= existencia) {
            ...
            if ($('#cbxBodega').val() > 0) { obj.append('stock', $('#cbxStock').val()); }
            $.ajax({
            ...
            messeges('warning','La cantidad a mover tiene que estar en el rango 1-' + $('#existencia').val());
        }
    } else {
    ...
    if (value > 0) {
        $.get('/entrada/listarStock', function (lstStock) {
            ...
            $.each(lstStock, function (key, item) {
                html += '<option value="' + item.iidstock + '">' + item.nombrestock + '</option>';
            });
            html += '</select>'
...
function eliminarInventario(id) {
    ConfirmarEliminacion('/bodegainventario/eliminarInventario?id=' + id, false
        , 'Recuerda que si tiene existencias se perderan al eliminar!','Eliminar inventario!');
}
    ...
    if (!isNaN($('#txtcantidad').val())) {
        if ($('#txtcantidad').val() >= 0) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.get('/bodegainventario/editarExistenciaInventario?id=' + $('#txtinventario').val() + '&cantidad=' + $('#txtcantidad').val(), function (respuesta) {
                        if (respuesta == 'ok') {
...
function listarProductos() {
    $.get('/BodegaInventario/listarProductos', function (data) {
        ...
        $.each(data, function (key, item) {
            ...
            html += '<td>';
            html += '<a title="Cambiar existencias" class="btn-sm btn-success" href="#" data-toggle="modal" onclick="existencia(' + item.iidproducto + ')" data-target="#modalExistencia"><i class="fas fa-edit"></i></a>';
            html += '</td>';
            html += '</tr>';
        });
        html += '</tbody>';
