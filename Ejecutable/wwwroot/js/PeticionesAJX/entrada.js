...
function callTable() {
    paintTable('/Entrada/listEntrada', ['No.', 'producto','proveedor', 'precio', 'f. inicio crédito', 'f. final','entrada'],
        ['iidentrada', 'descripcionproducto','proveedor', 'preciocompra', 'fechainiciocredito', 'fechavencimiento', 'entrada'],
        'iidentrada', '', true, false);
};
    ...
    $.get('/bodegainventario/listBodega', function (data) {
        if (data.length > 0) {
            $.each(data, function (key, item) {
                ...
                html += '<input type="text" onkeyup="calcularCantidad()" autocomplete="off" class="form-control data inputBodegas" name="bodega' + item.iidbodega + '" placeholder="Cantidad a guardar en ' + item.nombrebodega + '"/>'
                html += '</div>'
                html += '</div>'
                //ubicacion
                html += '<div class="col-lg-3">'
...
function deleteInfo(id) {
    messegeConfirm('/entrada/deleteEntrada?id=' + id, 'Estas por eliminar!', '¿Seguro deseas eliminar la entrada?', undefined,
