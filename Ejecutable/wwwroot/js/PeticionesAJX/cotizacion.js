...
function callTable() {
  paintTable("/Cotizacion/listDetalleCotizacion", [
    "producto",
    "um",
    "cant",
    "P.U",
...
function paintTable(link, headboard) {
  ...
  $.get(link, function (data) {
    if (data.length > 0) {
      ...
      document
        .getElementById("btnCancelarCotizacion")
...
function abrirModalProducto(tipo) {
  ...
  $.get(url, function (data) {
    ...
    $.each(data, function (objeto, propiedad) {
      let subexistencia = propiedad.subexistencia;
      ...
      html +=
        ...
        "</a> ";
      if (propiedad.nombresubunidad != "No tiene") {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data > 0) {
          ...
          messeges(
            "error",
            "Se ha superado el límite de cotización establecido en el sistema"
          );
        } else {
