...
function sendData() {
  if (validateEmpty()) {
    ...
    for (let i = 0; i < checkes.length; i++) {
      //los vamos a recorrer usando for
      if (checkes[i].checked == true) {
        // si el input esta marcado
    ...
    sendDataController(
      "/tipousuario/saveTipoUsuario",
      frm,
      "Este rol ya existe en la base de datos."
    );
  } else {
...
function listarPaginas() {
  $.get("/tipousuario/listPaginas", function (data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      html += "<td>";
      html +=
        '<input type="checkbox" class="paginasCheck" id="chk' +
        data[i].iidpagina +
        '" value="check' +
        data[i].iidpagina +
        '"/>';
