...
function GenerarMenu() {
  $.get("/Login/generarMenu", function (data) {
    document.getElementById("btnCerrarSesion").innerHTML =
      '<a class="btn btn-dark" title="Cerrar sesion" style="border-color:#808080;" href="#" onclick="cerrarSesion()"><i class="fas fa-power-off"></i> </a>';
    var html = "";
    if (data != null && data.length != 0) {
      $.each(data, function (key, item) {
        html +=
          '<a class="list-group-item list-group-item-action bg-dark text-light" href="/' +
          item.controlador +
          "/" +
          item.accion +
          '"><i class="' +
