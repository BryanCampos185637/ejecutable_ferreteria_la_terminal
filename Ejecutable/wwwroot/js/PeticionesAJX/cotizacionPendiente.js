...
function paintTable() {
  $.get("/cotizacionpendiente/lstcotizacion", function (data) {
    ...
    $.each(data, function (obj, item) {
      ...
      html +=
        '<a class="btn-sm btn-primary" title="Editar la lista de productos" data-toggle="modal" data-target="#modalListaProductos" href="#" onclick="verListaDeProductos(' +
        item.iidcotizacion +
        ')">Editar productos <i class="fas fa-edit"></i></a> ';
      html +=
        '<a class="btn-sm btn-primary" title="Facturar cotizacion" data-toggle="modal" data-target="#modalFactura" href="#" onclick="ObtenerNumCotizacion(' +
        item.iidcotizacion +
        ')">Facturar <i class="fas fa-file-invoice-dollar"></i></a>';
      html += "</td>";
      html += "</tr>";
    });
...
function createCookie(id) {
  $.get(
    "/cotizacionPendiente/getDetalleCotizacionByIdCotizacion?id=" + id,
    function (r) {
      if (r != null) console.log("OK");
  ...
  $.get(
    ...
    function (data) {
      document.getElementById("tituloLista").innerHTML =
        "<h4>Editar productos de la cotización No. " +
        data[0].nocotizacion +
        "</h4>";
      let html = "";
      let total = 0;
      ...
      html +=
        '<table class="table table-hover table-bordered table-responsive-md table-responsive-sm" id="listaCotizacion">';
      ...
      $.each(data, function (obj, item) {
        ...
        if (data.length > 1) {
          ...
          html +=
            '<a class="btn-sm btn-danger" title="Eliminar este producto" onclick="eliminarProducto(' +
            item.iidproducto +
            "," +
            id +
            "," +
        ...
        html += "</tr>";
        comi = comi + item.comision;
        desc = desc + item.descuento;
        total = total + item.total;

        //pintar data

        document.getElementById("tdDescuento").innerHTML =
          "<label>Descuento: $" + desc.toFixed(4) + "</label>";
        document.getElementById("tdComision").innerHTML =
          "<label>Comision: $" + comi.toFixed(4) + "</label>";
        document.getElementById("tdTotal").innerHTML =
          "<label>Total a pagar: $" + total.toFixed(4) + "</label>";
        document.getElementById("tdComisionConDescuento").innerHTML =
          "<label>Total comision: $" +
          parseFloat(comi - desc).toFixed(4) +
          "</label>";
      });
      html += "<tbody>";
      html += "</table>";
      ...
      $.ajax({
        ...
        success: function (e) {
          if (e == "ok") {
            ...
            messeges(
              "error",
              "Se ha superado el límite de facturas establecido en el sistema "
            );
          } else if (e == "finalCredito") {
            messeges(
              "error",
              "Se ha superado el límite de comprobantes de crédito fiscal establecido en el sistema "
            );
          } else {
            messeges("error", e);
          }
...
function clearData() {
  ...
  for (let i = 0; i < dataVista.length; i++) {
...
function eliminarProducto(id, idProducto, idCotizacion) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.get(
        "/cotizacionPendiente/eliminarProductoDeLaLista?id=" +
          id +
          "&idCotizacion=" +
          idCotizacion,
        function (data) {
...
function abrirModalCliente() {
  $.get("/cliente/listCliente", function (data) {
    let html = "";
    html +=
      '<table class="table table-hover table-bordered table-responsive-lg table-responsive-md table-responsive-sm" id="tablecliente">';
    html += '<thead class="thead-dark">';
    html += "<tr>";
    html += "<th>NOMBRE</th>";
    html += "<th>REGISTRO</th>";
    ...
    $.each(data, function (objeto, propiedad) {
      ...
      html += "<td>";
      html +=
        '<a class="btn-sm btn-primary" href="#" onclick="getCliente(' +
        propiedad.iidcliente +
        ')">seleccionar <i class="fas fa-check"></i></a> ';
      html += "</td>";
      html += "</tr>";
    });
    html += "</tbody>";
...
function abrirModalProducto(tipo) {
  ...
  $.get(url, function (data) {
    ...
    $.each(data, function (objeto, propiedad) {
      ...
      if (subexistencia > 0 && subexistencia != null)
        //si subexistencias es diferente de null
      ...
      if (subprecio > 0 && subprecio != null)
        //si subprecio es diferente de null
      ...
      html += '<td class="text-center">';
      if (existencia > 0) {
        //si es mayor a 0 se puede vender
        if (existencia > 1) {
          //SI LA EXISTENCIA ES MAYOR A 1 NO HAY NINGUNA VALIDACION
          html +=
            '<a title="Selecciona el producto con el precio original" class="badge badge-primary" href="#" onclick="getProducto(' +
            propiedad.iidproducto +
            ')">' +
            propiedad.nombreunidad +
            "</a> ";
          if (subexistencia > 0 && subexistencia != null) {
            //si la sub existencia existe podemos vender original y sub producto
            html +=
              '<a title="Selecciona el producto con el subprecio" class="badge badge-success" href="#" onclick="getSubProducto(' +
              propiedad.iidproducto +
              ')">' +
              propiedad.nombresubunidad +
              "</a> ";
          }
        } else if (existencia == 1) {
          //SI PRODUCTO SOLO TIENE 1 Y TIENE SUB PRODUCTO SE DEBE VALIDAR
          if (subexistencia <= 0 || subexistencia == null) {
            //SI NO TIENE SUB PRODUCTO SOLO SE AGREGA EL BOTON AZUL
            html +=
              '<a title="Selecciona el producto con el precio original" class="badge badge-primary" href="#" onclick="getProducto(' +
              propiedad.iidproducto +
              ')">' +
            ...
            //SI TIENE SUB PRODUCTO SE DEBE VALIDAR EL BOTON AZUL
            if (subexistencia >= equivalencia) {
              //SI LA EQUIVALENCIA ES MAYOR O IGUAL A LAS SUB EXISTENCIAS PUEDE VENDERSE EL ORIGINAL Y LAS SUB PIEZAS
              html +=
                '<a title="Selecciona el producto con el precio original" class="badge badge-primary" href="#" onclick="getProducto(' +
                propiedad.iidproducto +
                ')">' +
              ...
              //SI ES MENOR QUE LA EQUIVALENCIA SOLO EL SUB PRODUCTO
              html +=
                '<a title="Selecciona el producto con el subprecio" class="badge badge-success" href="#" onclick="getSubProducto(' +
                propiedad.iidproducto +
                ')">' +
        ...
        //si es menor a 0
        html +=
          '<a title="Este producto no cuenta con existencias" class="btn-sm btn-danger" href="#" onclick="mensajeError()">No hay <i class="fas fa-exclamation-triangle"></i></a> ';
      }
      html += "</td>";
      html += "</tr>";
    });
...
function getProducto(id) {
  ...
  $.get("/producto/getProductoById?id=" + id, function (data) {
    ...
    $("#subunidad").val(0);
    $.get(
      "/producto/ObtenerNombreUnidad?id=" + data.iidunidadmedida,
...
function getSubProducto(id) {
  $.get("/producto/getProductoById?id=" + id, function (data) {
    $.get(
      "/producto/ObtenerNombreUnidad?id=" + data.subunidad,
      function (nombreunidad) {
        $("#txtProducto").val("[" + nombreunidad + "] " + data.descripcion);
      }
    );
...
function addList() {
  if ($("#iidproducto").val() != "") {
    ...
    let idCotizacion = $("#txtIIDCOTIZACION").val();
    if (cantidad > 0) {
      if (cantidad <= existencias) {
        ...
        if (comision != "") frm.append("comision", comision);
        else frm.append("comision", 0);
        if (descuento != "") frm.append("descuento", descuento);
        else frm.append("descuento", 0);
        $.ajax({
            ...
            if (r > 0) {
              ...
              for (let i = 0; i < input.length; i++) {
                if (input[i].name == "cantidad") {
                  ...
                  input[i].name != "comision" &&
                  input[i].name != "descuento"
                ) {
                  input[i].value = "";
                }
              }
...
function descuentoGeneralAntesDeFacturar() {
  mostrarPosiblesProductosParDescuentoGeneral(
    "/cotizacionpendiente/getDetalleCotizacionByIdCotizacion?id=" +
      $("#cotizacion").val(),
...
function mostrarPosiblesProductosParDescuentoGeneral(link, headboard) {
  $.get(link, function (data) {
    ...
    let totalComisionProductosListados = 0;
    let totalFacturaProductosListados = 0;
    let totalProductosNoListados = 0;
    html +=
      '<table class="table table-hover table-bordered table-responsive-lg table-responsive-md table-responsive-sm" id="ListaProductosDescuento">';
    html += '<thead class="thead-dark">';
    html += "<tr>";
    let i = 0;
    ...
    $.each(data, function (obj, item) {
      if (item.subproducto == "NO") {
        //solo se muestra si es un subproducto
        let comisionUnitaria = item.comision / item.cantidad;
        let precioUnitarioConComision = comisionUnitaria + item.preciounitario;
        html += "<tr>";
        html += "<td>" + item.nombreproducto + "</td>";
        html += "<td>" + item.cantidad + "</td>";
        ...
        html += "</tr>";
        totalComisionProductosListados =
          totalComisionProductosListados + item.comision;
        totalFacturaProductosListados =
          totalFacturaProductosListados + item.total;
        totalPrecioConComision =
          totalPrecioConComision + item.precioconcomision;
    ...
    document.getElementById("txtComisionActual").value = parseFloat(
      totalComision - totalDescuento
    ).toFixed(4);
    document.getElementById("totalComisionProductosListados").value =
      totalComisionProductosListados;
    document.getElementById("totalFacturaProductosListados").value =
      totalFacturaProductosListados;
    document.getElementById("totalPrecioConComision").value =
      totalPrecioConComision;
    document.getElementById("txtActualNoListados").value =
      totalProductosNoListados.toFixed(4);
...
function calculateDescuentoGeneral() {
  ...
  let comisionActual = parseFloat(
    document.getElementById("txtComisionActual").value
  );
  let porcentajeDescuento = document.getElementById(
    "txtPorcentajeGeneral"
  ).value;
  let totalFacturaProductosListados = parseFloat(
    document.getElementById("totalFacturaProductosListados").value
  );
  let totalProductosNoListados = parseFloat(
    document.getElementById("txtActualNoListados").value
  );
  if (porcentajeDescuento != "") {
    //si esta diferente de vacio se procede a calcular
    if (!isNaN(porcentajeDescuento)) {
      if (porcentajeDescuento > 0) {
        //si el porcentaje es mayor a 0
        let descuento =
          (totalFacturaProductosListados / 100) * porcentajeDescuento;
        if (comisionActual == 0) {
          //si la comision es igual a 0 entonces se saca descuento al total de la factura
          document.getElementById("txtDescuentoGeneral").value =
            descuento.toFixed(4);
          let precioNuevo = totalActual - descuento + totalProductosNoListados;
          document.getElementById("txtTotalNuevo").value =
            precioNuevo.toFixed(4);
        } else {
          //si no se le saca descuento a la comision
          document.getElementById("txtDescuentoGeneral").value =
            descuento.toFixed(4);
          let comisionNueva = comisionActual - descuento;
          document.getElementById("txtComisionNuevo").value =
            comisionNueva.toFixed(4);
...
function aplicarDescuentoGeneral() {
  ...
  let txtContraseñaUsuario = document.getElementById(
