...
function callTable() {
  paintTable("/venta/listDetalleVenta", [
    "producto",
    "um",
    "cant",
    "P.U",
...
function paintTable(link, headboard) {
  ...
  $.get(link, function (data) {
    ...
    if (data.length > 0) {
      //si la data es mayor a 0 se desbloquean los botones
    ...
    $.each(data, function (obj, item) {
      ...
      if (!esSubProducto) html += "<td>" + item.unidadmedida + "</td>";
      else html += "<td>" + item.subproducto + "</td>";
      ...
      html += '<td class="text-center">';
      html +=
        '<a title="Eliminar producto de la lista" class="btn-sm btn-danger" href="#" onclick="deleteProducto(' +
        item.idlista +
    ...
    document.getElementById("totalComisionMenosDescuento").innerHTML =
      "$" + comisionmenosdescuento.toFixed(4);
    document.getElementById("totalComision").innerHTML =
      "$" + totalComision.toFixed(4);
    document.getElementById("totalDescuento").innerHTML =
      "$" + totalDescuento.toFixed(4);
    document.getElementById("totalVenta").innerHTML =
      "$" + totalPagar.toFixed(4);
  });
}
...
function limpiar() {
  ...
  for (var i = 0; i < inpust.length; i++) {
    ...
    if (nombre == "cantidad") {
      ...
      if (descuento > 0) inpust[i].style.borderColor = "#0000ff";
      else inpust[i].style.borderColor = "#ccc";
    } else if (nombre == "comision") {
      inpust[i].value = comision;
      if (comision > 0) inpust[i].style.borderColor = "#0000ff";
      else inpust[i].style.borderColor = "#ccc";
    } else {
      inpust[i].value = "";
    }
      ...
      $.ajax({
        ...
        success: function (e) {
          if (e == "ok") {
            ...
            messeges(
              "error",
              "Se ha superado el limite de facturas establecido en el sistema"
            );
          } else if (e == "finalCredito") {
            messeges(
              "error",
              "Se ha superado el limite de comprobantes de credito fiscal establecido en el sistema"
            );
          } else {
            messeges("error", e);
          }
...
function mostrarPosiblesProductosParDescuentoGeneral(link, headboard) {
  $.get(link, function (data) {
    ...
    var totalFactura = 0;
    var totalDescuento = 0;
    var totalComisionProductosListados = 0;
    var totalFacturaProductosListados = 0;
    var totalProductosNoListados = 0;
    html +=
...
function calculateDescuentoGeneral() {
  ...
  var comisionActual = parseFloat(
    document.getElementById("txtComisionActual").value
  );
  var porcentajeDescuento = document.getElementById(
    "txtPorcentajeGeneral"
  ).value;
  var totalFacturaProductosListados = parseFloat(
    document.getElementById("totalFacturaProductosListados").value
  );
  var totalProductosNoListados = parseFloat(
    document.getElementById("txtActualNoListados").value
  );
  if (porcentajeDescuento != "") {
    ...
    if (!isNaN(porcentajeDescuento)) {
      if (porcentajeDescuento > 0) {
        ...
        var descuento =
          (totalFacturaProductosListados / 100) * porcentajeDescuento; //sacamos el descuento al total de los productos listados
        if (comisionActual == 0) {
...
function aplicarDescuentoGeneral() {
  ...
  var txtContraseñaUsuario = document.getElementById(
