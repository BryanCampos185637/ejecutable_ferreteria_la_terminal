...
function contadorDeProductos() {
  $.get("/producto/TotalProductos", (data) => {
    $("#divTotalProductos").html(
      "<span>Total de registros " + data + "</span>"
    );
  });
}
...
function edit(id) {
  ...
  $.get("/producto/getProductoById?id=" + id, function (data) {
    ...
    var precioConUtilidad =
      parseFloat(data.preciocompra) + parseFloat(data.ganancia);
    ...
    if (data.subunidad > 0) {
      ...
      var subprecioConUtilidad =
        parseFloat(data.subpreciounitario) + parseFloat(data.subganancia);
...
function calculatePrice() {
  ...
  if (!isNaN(precioCompra)) {
    if (parseFloat(precioCompra) > 0) {
      var ivaCompra =
        parseFloat(precioCompra) * 0.13 + parseFloat(precioCompra);
      document.getElementById("ivaProducto").value = ivaCompra.toFixed(4);
      if (parseFloat(ganancia) > 0) {
        ...
        document.getElementById("Precioutilidad").value =
          precioMasUtilidad.toFixed(4);
        //caluculamos iva
        var iva = parseFloat(precioMasUtilidad) * 0.13;
        document.getElementById("iva").value = iva.toFixed(4);

        var precioFinal =
          parseFloat(precioCompra) + parseFloat(iva) + parseFloat(utilidad);
  ...
  if (
    $("#subiidunidadmedida").val() != null ||
    $("#subiidunidadmedida").val() > 0
  ) {
    calculateSubPrice();
  }
}
...
function calculateSubPrice() {
  ...
  var subprecio = parseFloat(precioCOmpraOriginal) / parseFloat(equivalencia);
  $("#subpreciocompra").val(subprecio.toFixed(4)); //divido el precio original entre la equivalencia
    ...
    if (!isNaN(precioCompra)) {
      if (parseFloat(precioCompra) > 0) {
        if (parseFloat(ganancia) > 0) {
          ...
          //calculo iva
          var precioMasUtilidad =
            parseFloat(precioCompra) + parseFloat(utilidad);
          //pintamos precio con utilidad
          document.getElementById("subPrecioutilidad").value =
            precioMasUtilidad.toFixed(4);
          //caluculamos iva
          var iva = parseFloat(precioMasUtilidad) * 0.13;
          ...
          var precioFinal =
            parseFloat(precioCompra) + parseFloat(iva) + parseFloat(utilidad);
          document.getElementById("subprecioventa").value =
            precioFinal.toFixed(4);
        } else {
...
function sendData() {
  if (validateEmpty()) {
    ...
    if ($("#subiidunidadmedida").val() == $("#iidunidadmedida").val())
      messeges(
        "warning",
        "La sub unidad no puede ser igual a la unidad original."
      );
    else
      guardarProducto(
        "/producto/saveProducto",
        frm,
        "El codigo ingresado ya esta asociado a un producto"
      );
  } else {
